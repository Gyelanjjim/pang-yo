...
<P align="center">📆 (1차) 2023.05.29 ~ 2023.06.25.
...
- `오븐`관리자만 title, description을 수정할 수 있도록 권한에 대한 `username 유효성 검사`

3. **`빵` 검색 서비스**

- 검색 조건 선택에 대한 클릭 수를 줄이고 사용자 편의성을 높이기 위해 하나의 검색값(value)으로 title 또는 body에 값이 있으면 해당 `빵`을 모두 조회
- `빵`이 대량으로 존재할 때 한 번에 모두 불러오지 않고 클라이언트에서 정한 일정 페이지 당 일정 개수(없으면 8개)의 `빵`을 조회

4. **특정 `빵` 삭제 서비스**

- 특정 `빵` 삭제시 관련 댓글이나 투표 정보를 남길 수 있게 soft delete 방식으로 삭제
- 특정 `빵`이 위치한 `오븐`의 관리자 또는 특정 `빵`작성자일 때 삭제할 수 있도록 username 일치 여부를 조회

💻 FE:

1. 일정 수 이상의 투표를 받은 `빵`에 대하여 PostCard 컴포넌트에서 voteScore값에 따라 배경색을 바꾸어 잘 구운 빵과 탄 빵 컨셉의 서비스 구현하여 비추천 받은 빵의 내용은 신경쓰지 않도록 함.
